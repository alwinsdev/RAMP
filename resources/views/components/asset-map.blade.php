...
{{--
    Asset Intelligence Map (flagship visualization). Renders colour-coded markers with
    clustering, a heatmap toggle, info windows, and auto-fit to the (role-scoped,
    filtered) marker set. Reused full-screen and embedded on the dashboard.
--}}
<div class="relative overflow-hidden rounded-xl border border-hairline bg-surface shadow-[var(--shadow-card)]">
    ...
    @if ($mapsKey !== '')
        <div wire:ignore
             x-data="assetIntelMap({ key: @js($mapsKey), mapId: @js($mapId), embedded: {{ $embedded ? 'true' : 'false' }}, markers: @js($markers) })"
            ...
            <div x-show="failed" x-cloak class="flex flex-col items-center justify-center gap-2 text-ink-soft" style="height: {{ $height }};">
                ...
                <p class="text-sm">Map could not be loaded. Check the Google Maps API key &amp; referrer.</p>
            </div>
        </div>
    @else
        <div class="flex flex-col items-center justify-center gap-2 text-ink-soft" style="height: {{ $height }};">
            <svg class="h-8 w-8 text-ink-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21s-7-5.5-7-11a7 7 0 0 1 14 0c0 5.5-7 11-7 11z"/><circle cx="12" cy="10" r="2.5"/></svg>
            <p class="text-sm">Add a <span class="font-semibold">GOOGLE_MAPS_API_KEY</span> to enable the Asset Intelligence Map.</p>
            <p class="text-xs text-ink-muted">{{ count($markers) }} assets ready to plot.</p>
        </div>
    @endif
</div>
